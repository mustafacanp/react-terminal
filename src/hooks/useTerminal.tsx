...
export const useTerminal = () => {
	...
	const commands = {
		help: async () => {
			...
			cout(['Usable Commands:', ...availableCommands].join('&#09;'), false);
		},
	...
	const handleUpArrow = useCallback(
		(e: KeyboardEvent) => {
			...
			if (state.currentLineFromLast < state.previousCommands.length) {
				...
				setState(prev => ({ ...prev, currentLineFromLast: newCurrentLine }));
				if (_prompt.current) {
					_prompt.current.setValue(
	...
	const handleDownArrow = useCallback(() => {
		if (state.currentLineFromLast > 1) {
			...
			if (_prompt.current) {
				_prompt.current.setValue(
					state.previousCommands[state.previousCommands.length - newCurrentLine]
				);
			}
		}
	}, [state.currentLineFromLast, state.previousCommands]);
