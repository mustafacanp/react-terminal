export const createTerminalCommands = (context) => {
  ...
  const resolveFileSystemPath = (rootFileSystem, currentFileSystem, currentPath, targetPath) => {
  ...
  return {
    ...
    cd: (sudo, input) => {
      ...
      const selected_file_or_dir = resolveFileSystemPath(state.fs, state.cfs, state.path, secondParam);
      ...
      if (is_file(selected_file_or_dir)) {
        ...
        if (!file.sudo || sudo) {
          ...
          if (pathParts.length === 1) {
            ...
            cout(`rm: removing files from nested directories not yet implemented`);
            return;
          }
          cout();
